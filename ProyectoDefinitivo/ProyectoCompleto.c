...
int main(void)
{
    ...
    // Configurar tipo de interrupción: flanco de bajada (botón con pull-up activo)
    GPIOIntTypeSet(GPIO_PORTJ_BASE, GPIO_PIN_0 | GPIO_PIN_1, GPIO_FALLING_EDGE);
    // Habilitar interrupciones para los pines 0 y 1 del puerto J
    GPIOIntEnable(GPIO_PORTJ_BASE, GPIO_PIN_0 | GPIO_PIN_1);
    // Registrar la función de ISR para el puerto J
    GPIOIntRegister(GPIO_PORTJ_BASE, intBoton);
    //Habilitar interrupción del pto J (No necesaria si se ha hecho la línea anterior)
    IntEnable(INT_GPIOJ);
    ...
    while(1) {
        ...
        if (POSX >= 20 && POSX <= 74 && POSY >= 10 && POSY <= 64)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_POWER.data, SIGNAL_MANDO_POWER.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, encender_data_Ana, size, volumen);
        }
        else if (POSX >= 20 && POSX <= 110 && POSY >= 70 && POSY <= 160)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_LA1.data, SIGNAL_MANDO_LA1.length, RELOJ);
                size[0] = haspulsadoSizeAna;
                size[1] = la1Size_Ana;
                loadPlayAudio(last_addr,haspulsado_data_Ana, la1_data_Ana, size, volumen);
        }
        ...
        else if (POSX >= 140 && POSX <= 230 && POSY >= 70 && POSY <= 160)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_LA2.data, SIGNAL_MANDO_LA2.length, RELOJ);
                size[0] = haspulsadoSizeAna;
                size[1] = la2Size_Ana;
                loadPlayAudio(last_addr,haspulsado_data_Ana, la2_data_Ana, size, volumen);
        }
        else if (POSX >= 260 && POSX <= 350 && POSY >= 70 && POSY <= 160)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_ANTENA3.data, SIGNAL_MANDO_ANTENA3.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, antena3_data_Ana, size, volumen);
        }
        else if (POSX >= 20 && POSX <= 110 && POSY >= 170 && POSY <= 260)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_LA4.data, SIGNAL_MANDO_LA4.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, cuatro_data_Ana, size, volumen);
        }
        else if (POSX >= 140 && POSX <= 230 && POSY >= 170 && POSY <= 260)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_TELECINCO.data, SIGNAL_MANDO_TELECINCO.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, telecinco_data_Ana, size, volumen);
        }
        else if (POSX >= 260 && POSX <= 350 && POSY >= 170 && POSY <= 260)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_LASEXTA.data, SIGNAL_MANDO_LASEXTA.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, lasexta_data_Ana, size, volumen);
        }
        else if (POSX >= 400 && POSX <= 454 && POSY >= 30 && POSY <= 84)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_VOLUP.data, SIGNAL_MANDO_VOLUP.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, subir_data_Ana, size, volumen);
        }
        else if (POSX >= 400 && POSX <= 454 && POSY >= 110 && POSY <= 164)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_VOLDOWN.data, SIGNAL_MANDO_VOLDOWN.length, RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, bajar_data_Ana, size, volumen);
        }
        else if (POSX >= 400 && POSX <= 454 && POSY >= 190 && POSY <= 244)
        {
            ...
            ir_send_raw(SIGNAL_MANDO_MUTE.data, SIGNAL_MANDO_MUTE.length,RELOJ);
                ...
                loadPlayAudio(last_addr,haspulsado_data_Ana, silenciar_data_Ana, size, volumen);
        }
    }
