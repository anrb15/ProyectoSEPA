...
void InterfazMando(void)
{
    ...
    // Arrays de posiciones y tamaÃ±os
...
// Delay en microsegundos (SysCtlDelay tarda 3 ciclos por iteraciÃ³n)
...
// Enviar seÃ±al (raw: seÃ±al medida en duraciones MARK/SPACE directamente)
...
// Manejador de interrupciones botones para volumen
void intBoton(void)
{
    // Lógica de cambio de estado
    if(GPIO_PIN_0 & (GPIOIntStatus(GPIO_PORTJ_BASE,0))) { // Botón 1 pulsado
        SysCtlDelay(800000); // Debouncing (20ms)
        while(B1_ON);
        SysCtlDelay(800000); // Debouncing (20ms)
        volumen+=50;
        if (volumen>255) volumen = 255;
        GPIOIntClear(GPIO_PORTJ_BASE, GPIO_PIN_0); // Limpiar flags de interrupción
    }
    else if(GPIO_PIN_1 & (GPIOIntStatus(GPIO_PORTJ_BASE,0))) { // Botón 2 pulsado
        SysCtlDelay(800000); // Debouncing (20ms)
        while(B2_ON);
        SysCtlDelay(800000); // Debouncing (20ms)
        volumen-=50;
        if (volumen<0) volumen = 0;
        GPIOIntClear(GPIO_PORTJ_BASE, GPIO_PIN_1); // Limpiar flags de interrupción
    }
}
